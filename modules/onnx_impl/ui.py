...
def create_ui():
    ...
    from modules.shared import opts, cmd_opts, refresh_checkpoints
                ...
                with gr.TabItem("Manage cache", id="manage_cache"):
                    ...
                    with gr.Row():
                        def remove_cache_onnx_converted(dirname: str):
                            ...
                            print(f"ONNX converted cache of '{dirname}' is removed.")
                    ...
                    with gr.Column():
                        ...
                        def remove_cache_optimized(dirname: str, s: str):
                            ...
                            print(f"Olive processed cache of '{dirname}' is removed: width={size[0]}, height={size[1]}")
                        with gr.Row():
                    ...
                    def cache_update_menus(query: str):
                        ...
                        if checkpoint_info is None:
                            print(f"Could not find checkpoint object for '{query}'.")
                            return
                            ...
                            def sd_save():
                                for k, v in sd_configs.items():
                                    with open(os.path.join(sd_config_path, k), "w", encoding="utf-8") as file:
                                        json.dump(v, file)
                                print("Olive: config for SD was saved.")
                            ...
                            def sdxl_save():
                                for k, v in sdxl_configs.items():
                                    with open(os.path.join(sdxl_config_path, k), "w", encoding="utf-8") as file:
                                        json.dump(v, file)
                                print("Olive: config for SDXL was saved.")
